...
class V2TransfersCreatePostRequestInnerDestinationsInnerTransportersInner(BaseModel):
    """
    ...
    def to_dict(self) -> Dict[str, Any]:
        ...
        * `None` is only added to the output dict for nullable fields that
        ...
        # override the default output from pydantic by calling `to_dict()` of each item in transporter_details (list)
        _items = []
        if self.transporter_details:
            for _item_transporter_details in self.transporter_details:
                if _item_transporter_details:
                    _items.append(_item_transporter_details.to_dict())
            _dict['transporterDetails'] = _items
        return _dict
    ...
    def from_dict(cls, obj: Optional[Dict[str, Any]]) -> Optional[Self]:
        ...
        _obj = cls.model_validate({
            "transporterId": obj.get("transporterId"),
            "phoneNumberForQuestions": obj.get("phoneNumberForQuestions"),
            "transporterDetails": [V2TransfersCreatePostRequestInnerDestinationsInnerTransportersInnerTransporterDetailsInner.from_dict(_item) for _item in obj["transporterDetails"]] if obj.get("transporterDetails") is not None else None
        })
        return _obj
