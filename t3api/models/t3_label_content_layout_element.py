...
class T3LabelContentLayoutElement(BaseModel):
    """
    ...
    @field_validator('vertical_paragraph_alignment')
    def vertical_paragraph_alignment_validate_enum(cls, value):
        """Validates the enum"""
        if value is None:
            return value

        if value not in set(['TOP', 'CENTER', 'BOTTOM']):
            raise ValueError("must be one of enum values ('TOP', 'CENTER', 'BOTTOM')")
        return value
